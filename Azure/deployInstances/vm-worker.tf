resource "azurerm_network_interface" "nic_worker" {
    ...
    ip_configuration {
        ...
        public_ip_address_id          = azurerm_public_ip.public_ip_worker[count.index].id
    }
}

resource "azurerm_public_ip" "public_ip_worker" {
    count               = var.nb_worker
...
resource "azurerm_linux_virtual_machine" "vm_worker" {
    ...
    resource_group_name = data.azurerm_resource_group.rg_main.name
    location            = data.azurerm_resource_group.rg_main.location
    size                = var.profil_worker
    admin_username      = "adminazure"
    computer_name       = "worker-${count.index}"
    network_interface_ids = [
        azurerm_network_interface.nic_worker[count.index].id,
    ]
