...
echo "[k8sMasters]" > /Users/eq29753/Documents/Github/deployKubernetes/containerd/hosts
for IP_ADDRESSES_MASTER in $IP_ADDRESSES_MASTER; do
    hostname_master="kubernetes-master-$(printf "%02d" $num_master)"
    echo "${hostname_master} ansible_host=$IP_ADDRESSES_MASTER ansible_user=ubuntu" >> /Users/eq29753/Documents/Github/deployKubernetes/containerd/hosts
    num_master=$((num_master + 1))
done

# Écrit les adresses IP des workers dans le fichier Ansible hosts.
echo "[k8sWorkers]" >> /Users/eq29753/Documents/Github/deployKubernetes/containerd/hosts
for IP_ADDRESSES_WORKERS in $IP_ADDRESSES_WORKERS; do
    hostname_worker="kubernetes-worker-$(printf "%02d" $num_worker)"
    echo "${hostname_worker} ansible_host=$IP_ADDRESSES_WORKERS ansible_user=ubuntu" >> /Users/eq29753/Documents/Github/deployKubernetes/containerd/hosts
